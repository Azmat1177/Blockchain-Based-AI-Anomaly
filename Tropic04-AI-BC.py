...
class TelegramManager:
    ...
    def maybe_send_daily_csv(self, data_logger: DataLogger,
        ...
        caption  = (
            ...
            f"Time (UTC): `{datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}`\n"
            f"File hash (SHA-256): `{csv_hash[:16]}…` _(stored on blockchain)_\n"
...
class BlockchainManager:
    ...
    def submit_normal_reading(self, temperature: float, humidity: float) -> str | None:
        ...
        temp_f = int(temperature * 100)
        hum_f  = int(humidity    * 100)
    ...
    def submit_anomaly_reading(self, temperature: float, humidity: float,
        ...
        temp_f          = int(temperature * 100)
        hum_f           = int(humidity    * 100)
        anom_value_f    = int(anomaly["anomaly_value"]   * 100)
        threshold_f     = int(anomaly["threshold_value"] * 100)
    ...
    def store_csv_hash(self, csv_hash: str, timestamp_s: int) -> str | None:
        """Record daily CSV hash on-chain for integrity verification."""
        sender = self.get_sender_address()
        result = self._rpc_call("unsafe_moveCall", [
            sender, PACKAGE_ID, "DHT22Monitor", "store_csv_hash",
            [],
            [STORAGE_OBJECT, self._sv(DEVICE_ID),
             self._sv(csv_hash), str(timestamp_s), CLOCK_OBJECT_ID],
            GAS_OBJECT_ID, "10000000",
        ])
        if not result or "error" in result:
            logger.error(f"CSV hash store error: {result}")
            return None
        return self._execute(result["result"]["txBytes"])
...
class DHT22Monitor:
    ...
    def __init__(self):
        ...
        # State for recovery detection
        self._in_critical        = False
        self._critical_start     = 0.0
        self._critical_breach    = "UNKNOWN"
    ...
    def monitor_loop(self):
        ...
        while self.running:
            try:
                ...
                logger.info(f"Reading #{self.reading_counter + 1} — "
                ...
                if anomaly:
                    logger.info(
                        ...
                        f"consecutive_violations={anomaly['consecutive_violations']}  "
                ...
                if anomaly and anomaly["is_anomaly"] and BLOCKCHAIN_SUBMIT_ANOMALY_ALWAYS:
                    tx_digest = self.blockchain.submit_anomaly_reading(
                        temp, hum, anomaly, data_hash, tropic_sig
                    )
                    if tx_digest:
                        logger.info(f"  Anomaly tx: {tx_digest}")

                elif self._should_submit_normal():
                    tx_digest = self.blockchain.submit_normal_reading(temp, hum)
                    if tx_digest:
                        logger.info(f"  Normal tx:  {tx_digest}")

                # ── 5. Telegram — CRITICAL alerts only ───────────────────
                if anomaly and self.detector.should_send_critical_alert(anomaly):
                    if not self._in_critical:
                        # Transition: normal → critical
                        self._in_critical     = True
                        self._critical_start  = time.time()
                        self._critical_breach = self._determine_breach_type(temp, hum)
                    ...
                    # else: already in critical state, suppress repeat alerts

                elif self._in_critical:
                    # Transition: critical → normal (recovery)
                    duration = time.time() - self._critical_start
                    logger.info(f"Recovery detected after {duration:.0f}s")
                    self.telegram.send_recovery(temp, hum,
                                                self._critical_breach, duration)
                    self._in_critical    = False
                    self._critical_start = 0.0
